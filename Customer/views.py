...
def index(request):
    ...
    return render(request,'index.html',{'form' : form,'username':name})
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if data.is_valid():
            ...
            return render(request, 'home.html', {'hotels': all})
    name = User.objects.get(username=request.user.username)
    return render(request,'home.html',{'hotels' : all, 'search' : search, 'username':name})
...
def reserve(request,id):
    ...
    name = User.objects.get(username=request.user.username)
    return render(request,'reserve.html',{'hotels' : all ,  'all_rooms':all_rooms,'username':name})
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (not existing_user) and (len(prev_customer)==0) :
